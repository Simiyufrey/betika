...
if __name__ =="__main__":
    ...
    if token is not None:
       print(len(betslip))
       betslip=[bet for n,bet in enumerate(betslip) if n %15 ==0 ]
       print(len(betslip))
       stake=input("Enter bet amount: ")
       url3="https://api.betika.com/v2/bet"
      
       place_bet(url3,betslip,stake,token,total_odds)
    # read_contacts("numbers.txt")
    # file=open("balances.txt","w")
    # for i in range(1):
    #     for number in phone_numbers:
    #         token=login(os.path.join(base_url,l_url),data={"mobile":f"{number}","password":"1212","remember":True,"src":"MOBILE_WEB"})
    #         print(f"{number} balance {balance}")
    #         file.write(f"{number} balance {balance}\n")
    #         if token:
    #             data={"amount":str(balance),"app_name":"MOBILE_WEB","token":token}
    #             if balance > 50 and balance !=294:
    #                 withdraw(os.path.join(base_url,w_url),data,token)
    #             else:
    #                 print("Balance is 294 or less than 50 ",balance)
    #             token=""
    #     time.sleep(1)
    # file.close()
